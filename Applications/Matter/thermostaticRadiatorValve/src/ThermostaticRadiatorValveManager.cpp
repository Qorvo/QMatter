...
int16_t ThermostaticRadiatorValveManager::GetLocalTemperature()
{
    ...
    temperatureMeasurement_GetTemperatureValue(&temp);
...
void ThermostaticRadiatorValveManager::SetSystemMode(ThermostaticRadiatorValveManager::SystemMode_t aSystemMode)
{
    SystemLayer().ScheduleLambda([aSystemMode] {
        Thermostat::Attributes::SystemMode::Set(QPG_THERMOSTATIC_ENDPOINT_ID, (uint8_t) aSystemMode);

        ChipLogError(NotSpecified, "SetSystemMode value %d", (uint8_t) aSystemMode);
    });
}

uint8_t ThermostaticRadiatorValveManager::GetTemperatureDisplayMode(void)
{
    uint8_t value;
...
void ThermostaticRadiatorValveManager::SetTemperatureDisplayMode(uint8_t aMode)
{
    SystemLayer().ScheduleLambda([aMode] {
        ThermostatUserInterfaceConfiguration::Attributes::TemperatureDisplayMode::Set(QPG_THERMOSTATIC_ENDPOINT_ID,
                                                                                      (uint8_t) aMode);
