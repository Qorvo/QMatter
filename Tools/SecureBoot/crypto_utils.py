...
def signMessage(message, privateKey, curve=NIST256p, hashfunc=hashlib.sha256, deterministic=False) -> bytes:
    """ Sign a arbitrary message with privateKey"""
    privKeyInstance = SigningKey.from_string(privateKey, curve=curve)
    if deterministic:
        return privKeyInstance.sign_deterministic(message, hashfunc=hashfunc)
    else:
