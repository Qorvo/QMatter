 ...
 *
 * $Header: //depot/release/Embedded/Components/Qorvo/BaseUtils/v2.10.2.1/comps/gpRandom/src/gpRandom.c#1 $
 * $Change: 189026 $
...
{
    UInt8 seedByteIndex;
#if defined(GP_DIVERSITY_DEVELOPMENT)&& defined(GP_LOCAL_LOG)
    UInt32 startTimeStamp;
    UInt32 finishTimeStamp;

    startTimeStamp = gpSched_GetCurrentTime();
...
#if defined(GP_DIVERSITY_DEVELOPMENT)&& defined(GP_LOCAL_LOG)
    finishTimeStamp = gpSched_GetCurrentTime();
    GP_LOG_PRINTF("RANDOMIZED TSS: %lu TSF: %lu",0, (unsigned long)startTimeStamp, (unsigned long)finishTimeStamp);
#endif /* GP_DIVERSITY_DEVELOPMENT */
}
