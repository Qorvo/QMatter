 ...
 *
 * $Header: //depot/release/Embedded/Components/Qorvo/BaseUtils/v2.10.2.1/comps/gpNvm/src/gpNvm_RW_Kx_SubpagedFlash_v2.c#1 $
 * $Change: 189026 $
...
#define NVM_ROUND_UP_TO_MULTIPLE(n, k)                  ((n) + ((k) - (n) % (k)) % (k))
...
#define NVM_RECORD_BODY_SIZE(TokenLength, PayloadLength)    ((TokenLength) + (PayloadLength))

// Record size. Rounded up to a multiple of FLASH_WRITE_UNIT bytes.
#define NVM_RECORD_SIZE(TokenLength, PayloadLength)     NVM_ROUND_UP_TO_MULTIPLE(NVM_RECORD_HEADER_SIZE + (TokenLength) + (PayloadLength), FLASH_WRITE_UNIT)

#if defined(GP_DIVERSITY_DEVELOPMENT)
#if ! defined(__IAR_SYSTEMS_ICC__)
GP_COMPILE_TIME_VERIFY(NVM_RECORD_HEADER_SIZE==8);
#endif
...
#if defined(GP_DIVERSITY_DEVELOPMENT)
GP_COMPILE_TIME_VERIFY(GP_NVM_MAX_PAYLOADLENGTH <= (GP_NVM_MAX_BODYLENGTH - NVM_MIN_TOKEN_SIZE));
#endif //defined(GP_DIVERSITY_DEVELOPMENT)
...
#if defined(GP_DIVERSITY_DEVELOPMENT)
#if ! defined(GP_NVM_DIVERSITY_VARIABLE_SIZE)
GP_COMPILE_TIME_VERIFY(NVM_MAX_NUMBER_SECTORS < (1<<(8*sizeof(Nvm_PhySectorId_t))));
#endif //if ! defined(GP_NVM_DIVERSITY_VARIABLE_SIZE)
#endif // defined(GP_DIVERSITY_DEVELOPMENT)
...
typedef PACKED_PRE struct Nvm_TokenLut_Item_s
{
    ...
    Nvm_MaskedSeqNum_t        seqNum;              //Flash system - garbage/journaling
    UInt8                     tokenLength;         //Length of token sequence
...
#if defined(GP_DIVERSITY_DEVELOPMENT)
#if GP_NVM_NBR_OF_POOLS == 1
GP_COMPILE_TIME_VERIFY(NVM_MAX_NUMBER_SECTORS == (GP_NVM_POOL_1_NBR_OF_PHY_SECTORS));
#endif
#if GP_NVM_NBR_OF_POOLS == 2
GP_COMPILE_TIME_VERIFY(NVM_MAX_NUMBER_SECTORS == (GP_NVM_POOL_1_NBR_OF_PHY_SECTORS + GP_NVM_POOL_2_NBR_OF_PHY_SECTORS));
#endif
#if GP_NVM_NBR_OF_POOLS == 3
GP_COMPILE_TIME_VERIFY(NVM_MAX_NUMBER_SECTORS == (GP_NVM_POOL_1_NBR_OF_PHY_SECTORS + GP_NVM_POOL_2_NBR_OF_PHY_SECTORS + GP_NVM_POOL_3_NBR_OF_PHY_SECTORS));
#endif
#if GP_NVM_NBR_OF_POOLS == 4
GP_COMPILE_TIME_VERIFY(NVM_MAX_NUMBER_SECTORS == (GP_NVM_POOL_1_NBR_OF_PHY_SECTORS + GP_NVM_POOL_2_NBR_OF_PHY_SECTORS + GP_NVM_POOL_3_NBR_OF_PHY_SECTORS + GP_NVM_POOL_4_NBR_OF_PHY_SECTORS));
#endif
#endif //defined(GP_DIVERSITY_DEVELOPMENT)
...
static Bool Nvm_ReadRecordAndCheckCorrupted(UInt32 ptrTag, UInt32 pageAddrOffset, UInt8* pData, Bool* pIsLastRecord, UInt16* pRecordSize)
{
    ...
    *pIsLastRecord = false;
    ...
    // Check CRC.
    const UInt16 calculatedCrc = gpUtils_CalculateCrc(pData + NVM_RECORD_HEADER_CRC_BYTES, bodyLength + NVM_RECORD_HEADER_SIZE - NVM_RECORD_HEADER_CRC_BYTES);
    ...
    UIntLoop i;
    UInt16 sizeHeaderPlusBody = NVM_RECORD_HEADER_SIZE + bodyLength;
    UInt16 recordSize = NVM_RECORD_SIZE(tokenLength, payloadLength); /*bodyLenght includes payload and token*/

    /* initialize trailer padding */
    for (i = 0; i < ( recordSize - sizeHeaderPlusBody); i++)
    {
        pData[sizeHeaderPlusBody+i] = 0;
    }
...
static Bool Nvm_PhySector_IsCorrupt(Nvm_PhySectorId_t phySecId)
{
    ...
    for (pageId = 0; pageId < NVM_NUMBER_PAGES_PER_SECTOR; pageId++)
    {
        ...
        while (pageAddrOffset <= NVM_MAX_RECORD_OFFSET)
        {
             ...
             */
            UInt32 pData_32[NVM_ROUND_UP_TO_MULTIPLE(NVM_MAX_RECORD_SIZE, FLASH_WRITE_UNIT) / sizeof(UInt32) + 4];
            UInt8* pData = (UInt8*)pData_32;
            UInt32 ptrTag = NVM_PAGE_ADDROFFSET(phySecId,pageId) + pageAddrOffset;

            Bool isCorruptedRecord;
            Bool isLastRecord;
            isCorruptedRecord = Nvm_ReadRecordAndCheckCorrupted(ptrTag, pageAddrOffset, pData, &isLastRecord, &RecordSize);
...
)
{
    ...
    {
        UInt32 pageAddrOffset;
        UInt8 pData[NVM_RECORD_HEADER_SIZE];

        pageAddrOffset = NVM_SECTOR_HEADER_SIZE;
        while (pageAddrOffset <= NVM_MAX_RECORD_OFFSET)
        {
            UInt8 tokenLength;
            UInt8 payloadLength;
            ...
            pageAddrOffset += NVM_RECORD_SIZE(tokenLength, payloadLength);
        }
    }
...

                {
                    ...
                    if (lookUpResult == gpNvm_Result_DataAvailable)
                    {
                         ...
                         || (offsetInSector != SECTOROFFSET_UNPACK(lutItem->packed_sectorOffset)))
                        {
                            ...
                            SizeInActive += NVM_RECORD_SIZE(tokenLength, payloadLength);
                        }
                    }
                }
...
static Bool Nvm_LogSector_SalvageEntriesFromBrokenPhySectorToLogicalSector(gpNvm_PoolId_t poolId, Nvm_PhySectorId_t phySecId, Nvm_LogSectorId_t logSecId)
{
    ...
    for (pageId = 0; pageId < NVM_NUMBER_PAGES_PER_SECTOR; pageId++)
    {
        ...
        while (pageAddrOffset <= NVM_MAX_RECORD_OFFSET)
        {
             ...
             * add the padding up to a multiple of FLASH_WRITE_UNIT in this array
             */
            UInt32 pData_32[NVM_ROUND_UP_TO_MULTIPLE(NVM_MAX_RECORD_SIZE, FLASH_WRITE_UNIT) / sizeof(UInt32)];
            UInt8* pData_8 = (UInt8*)pData_32;
            ...
            Bool isCorruptedRecord;
            Bool isLastRecord;
            isCorruptedRecord = Nvm_ReadRecordAndCheckCorrupted(ptrTag, pageAddrOffset, pData_8, &isLastRecord, &recordSize);
...
void Nvm_TokenLuts_Invalidate(void)
{
    UInt8 i;
...
)
{
    ...
    for (i = 0; i < GP_NVM_NBR_OF_LOOKUPTABLE_HANDLES; i++)
    {
        ...
        if (Nvm_TokenLut_TokenBelongs(NVM_LUTHANDLE_TO_LUT(i), poolId, tokenLength, pToken))
        {
            /* reuse LUT */
            *pHandle = i;
...
)
{
    ...
    GP_ASSERT_DEV_EXT(tokenLength <= GP_NVM_MAX_TOKENLENGTH);
    GP_ASSERT_DEV_EXT(tokenLength > 0);
    ...
    item->tokenLength = tokenLength;
    item->seqNum = NVM_SEQNUM_INVALID;
...
)
{
    ...
    do
    {
        ...
        GP_ASSERT_DEV_EXT(lutItem->tokenLength >0);
        ...
        if ((lutItem->tokenLength >= tokenMaskLength)
        && (0 == MEMCMP(tokenContent, pTokenMask, tokenMaskLength)))
        {
            result = gpNvm_Result_NoUniqueMaskMatch;
            if((!strictMatching) || (strictMatching && lutItem->tokenLength == tokenMaskLength))
            {
                (*itemResult) = lutItem;
...
static void Nvm_Token_Update(Nvm_Token_t *lutItem, UInt16 offsetInSector, Nvm_LogSectorId_t logSecId, Nvm_MaskedSeqNum_t seqNum)
{
...
)
{
    ...
    for (logSecId = 0; logSecId < Nvm_Pool_LogSectors_GetNbr(poolIdSpec); logSecId++)
    {
        ...
        if (!Nvm_LogSector_InUse(poolIdSpec, logSecId))
        {
            continue;
        }
        ...
        for (pageId = 0; pageId < NVM_NUMBER_PAGES_PER_SECTOR; pageId++)
        {
            ...
            while (pageAddrOffset <= NVM_MAX_RECORD_OFFSET)
            {
                ...
                Nvm_Flash_Read(
                        Nvm_LogSector_GetPhyFlashOffset(poolIdSpec, logSecId) + offsetInSector,
                        NVM_RECORD_HEADER_SIZE,
                        pData
                );
                ...
                Nvm_Flash_Read(
                        Nvm_LogSector_GetPhyFlashOffset(poolIdSpec, logSecId) + offsetInSector + NVM_RECORD_HEADER_OFFSET_TO_TOKEN,
                        tokenLength,
                        token
                );
                ...
                if (Nvm_TokenLut_TokenBelongs(lut, poolIdSpec, tokenLength, token))
                {
                    ...
                    if (lookUpResult != gpNvm_Result_DataAvailable)
                    {
                        if (
                            ...
                            Nvm_IsActiveToken(poolIdSpec,tokenLength, token)         //check if token contains active data (shouldn't be called for unique token search to avoid infinite loop)
                           )
                        {
                            nrOfMatches++;
                    ...
                    if (lutItem && (
                        /* newly added to LUT */
                        (lutItem->seqNum == NVM_SEQNUM_INVALID)
                        /* more recent record found than the one in the LUT */
                        || (Nvm_Pool_SeqNum_Unmask(poolIdSpec, lutItem->seqNum) < Nvm_Pool_SeqNum_Unmask(poolIdSpec, seqNum))
                       )
                    ) {

                        Nvm_Token_Update(lutItem, offsetInSector, logSecId, seqNum);
                    }
                }
                        ...
                        )
{
    ...
    if (Nvm_Pool_IsUrgentDefragRequired(poolId))
    {
        GP_LOG_PRINTF("Perform defrag",0);
    ...
    {
        if (prevVersion.lookUpResult == gpNvm_Result_DataAvailable
        &&  (dataLength <= prevVersion.payloadLength))
        {
        ...
        else
        {
            ...
            newRecordSize = (NVM_RECORD_HEADER_SIZE + NVM_RECORD_BODY_SIZE(tokenLength, dataLength));
...
{
    const UInt16 sizeHeaderPlusBody = (NVM_RECORD_HEADER_SIZE + NVM_RECORD_BODY_SIZE(tokenLength,dataLength));
    ...
    Bool status = false;
    UInt16 crcValue;
    UInt32 pData_32[(NVM_MAX_RECORD_SIZE+sizeof(UInt32)-1)/sizeof(UInt32)];
    UInt8* pData_8 = (UInt8*)pData_32;
    UInt16 offsetInSector;
    Nvm_MaskedSeqNum_t maskedSeqNum;
    gpNvm_Result_t result = gpNvm_Result_DataAvailable;
    ...
    if (false == Nvm_Pool_Record_Allocate(poolId, recordSize, &logSecId, &pageId, gpNvm_DryRun_no))
    {
    ...
    GP_ASSERT_SYSTEM(sizeHeaderPlusBody <= (FLASH_PAGE_SIZE - NVM_SECTOR_HEADER_SIZE));
    ...
    crcValue = gpUtils_CalculateCrc( &pData_8[NVM_RECORD_HEADER_OFFSET_TO_TOKENLENGTH],sizeHeaderPlusBody - NVM_RECORD_HEADER_CRC_BYTES);
    Nvm_Buf_Put_UInt16(pData_8, NVM_RECORD_HEADER_OFFSET_TO_CRC, crcValue);

    /* initialize trailer padding */
    for (i = 0; i < (recordSize - sizeHeaderPlusBody); i++)
    {
        pData_8[sizeHeaderPlusBody+i] = 0;
    }

    status = Nvm_LogSector_StoreRecord(poolId, logSecId, pageId, recordSize, pData_32, &offsetInSector);
    ...
    if (prevVersion.lookUpResult == gpNvm_Result_DataAvailable)
    {
        ...
        Nvm_SectorLut_t *sectorLut = Nvm_Pool_SectorLut_Get(poolId);
        sectorLut[prevVersion.logSecId].InactiveSum += NVM_RECORD_SIZE(tokenLength, prevVersion.payloadLength);
    }
...
#define NBR_OF_IN_USE_WORDS  ((UINT8_MAX+1) / NBR_OF_BITS)
    ...
    do
    {
        ...
        if (!((lutItem->tokenLength >= tokenLength)
        && (0 == MEMCMP(tokenContent, pToken, tokenLength))))
        {
            continue;
        }
