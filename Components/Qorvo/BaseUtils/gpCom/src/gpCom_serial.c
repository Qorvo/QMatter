...
#ifdef GP_COM_DIVERSITY_SERIAL_SYN
        case gpCom_ProtocolSyn:
        {
            if(ComSynProtocol_ParseByte(rxbyte, state) == gpCom_ProtocolDone)
            {
                state->Com_protocol = gpCom_ProtocolInvalid;
            }
            break;
...
#endif // GP_COM_DIVERSITY_SERIAL_NO_SYN_NO_CRC
#ifdef GP_COM_DIVERSITY_SERIAL_SYN
            status =  ComSynProtocol_ParseByte(rxbyte, state);
            if (status == gpCom_ProtocolContinue)
            {
                state->Com_protocol = gpCom_ProtocolSyn;
                break;
            }
