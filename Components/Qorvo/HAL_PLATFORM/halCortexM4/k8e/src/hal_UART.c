...
static void halUart_RxCheckErrors(UInt8 uart)
{
    ...
    if (GP_WB_READ_UART_UNMASKED_RX_OVERRUN_INTERRUPT(uartBaseAddress))
    {
        uarterr |= BM(0);
    }

    if (GP_WB_READ_UART_UNMASKED_RX_PARITY_ERROR_INTERRUPT(uartBaseAddress))
    {
        uarterr |= BM(1);
    }

    if (GP_WB_READ_UART_UNMASKED_RX_FRAMING_ERROR_INTERRUPT(uartBaseAddress))
    {
        uarterr |= BM(2);
    }
    ...
    if(uarterr)
    {
        ...
        GP_LOG_SYSTEM_PRINTF("rx uart err:%x",0,uarterr);
        hal_UartComFlush(uart);
    }
    GP_ASSERT_DEV_INT(uarterr == 0);
