...
#ifdef GP_DIVERSITY_DEVELOPMENT
        // subtract whole mile seconds
        UInt32 diffBeforeSleep = beginSleepTimeUs - (xTaskGetTickCount() * portTICK_PERIOD_MS * MS_TO_US(1));
        // subtrack mikro seconds
        // systick countdown
        diffBeforeSleep -= (systickLoad - SysTick->VAL) / CPU_CLOCK_MHZ;
        // add start offset
        diffBeforeSleep += halFreeRTOS_TimerOffsetUs;
#endif
...
#ifdef GP_DIVERSITY_DEVELOPMENT
        completeSleepTimeUs = endSleepTimeUs - beginSleepTimeUs;
        if (completeSleepTimeUs > 1000)
        {
            GP_LOG_PRINTF("Sleep time expected:%ld, completed:%ld[us]", 0, xExpectedIdleTime, endSleepTimeUs - beginSleepTimeUs);
            GP_LOG_PRINTF("DB:%ld[us] DA:%ld[ticks]", 0,
                          diffBeforeSleep, // diff before sleep
                          diffLongTimeMs); // diff after sleep
        }
