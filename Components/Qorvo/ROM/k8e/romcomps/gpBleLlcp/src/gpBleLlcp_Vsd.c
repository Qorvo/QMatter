...
// VSD helpers
#ifdef GP_DIVERSITY_BLE_PER_TEST
static gpHci_Result_t Ble_SetVsdPER_TestModeHelper(gpHci_CommandParameters_t* pParams, gpBle_EventBuffer_t* pEventBuf);
#endif /* GP_DIVERSITY_BLE_PER_TEST */
...
#ifdef GP_DIVERSITY_BLE_PER_TEST
gpHci_Result_t Ble_SetVsdPER_TestModeHelper(gpHci_CommandParameters_t* pParams, gpBle_EventBuffer_t* pEventBuf)
{
    BLE_SET_RESPONSE_EVENT_COMMAND_COMPLETE(pEventBuf->eventCode);

    Ble_SetVsdPER_TestMode(pParams->SetVsdTestParams.value[0]);
    // Tell gpHal to limit the connection event duration to 1 MtoS PDU
    // Note: access code validation parameters can be overruled afterwards
    gpHal_BleStoreACValidationParameters(BLE_PER_TEST_MODE_IDLE != Ble_GetVsdPER_TestMode(), false, 0, false, 0, 0);
...
gpHci_Result_t gpBle_SetVsdTestParams(gpHci_CommandParameters_t* pParams, gpBle_EventBuffer_t* pEventBuf)
{
    ...
    {
#ifdef GP_DIVERSITY_BLE_PER_TEST
        case gpBle_SetVsdPER_TestMode: return Ble_SetVsdPER_TestModeHelper( pParams, pEventBuf);
#endif /* GP_DIVERSITY_BLE_PER_TEST */
...
#ifdef GP_DIVERSITY_BLE_PER_TEST
gpHci_Result_t gpBle_VsdSetAccessCodeValidationParameters(gpHci_CommandParameters_t* pParams, gpBle_EventBuffer_t* pEventBuf)
{
    BLE_SET_RESPONSE_EVENT_COMMAND_COMPLETE(pEventBuf->eventCode);
    gpHal_BleStoreACValidationParameters(    BLE_PER_TEST_MODE_IDLE != Ble_GetVsdPER_TestMode(),
                                             pParams->VsdSetACValidationParameters.validity,
                                             pParams->VsdSetACValidationParameters.validation_n,
                                             pParams->VsdSetACValidationParameters.fake_preamble_present,
                                             pParams->VsdSetACValidationParameters.fake_preamble_n,
                                             pParams->VsdSetACValidationParameters.validation_threshold);
