...
// Vendor Specific test helper functions
gpHci_Result_t Ble_SetVsdWinoffset(UInt16 VSWinoffset);
...
void Ble_SetVsdConnReqWinSize(UInt8 WinSize);
void Ble_SetVsdAutomaticFeatureExchangeEnable(Bool enable);
void Ble_SetVsdSetSleepClockAccuracy(UInt8 SCA);
UInt8 Ble_GetVsdInitiatorVirtualConnId(UInt8 connId);
...
gpHci_Result_t Ble_SetVsdLLCPTriggerForAnchorMove(gpHci_ConnectionHandle_t HciConnHandle, UInt16 OffsetAgainstCurrentSchedule);
...
void Ble_SetVsdPER_TestMode(UInt8 PER_TestMode);
UInt8 Ble_GetVsdPER_TestMode(void);
void Ble_SetPER_TestModeLastRxedData(UInt8 data);
