...
/* device name */
static uint8_t gapValDn[ATT_DEFAULT_PAYLOAD_LEN] = DEFAULT_DEVICE_NAME;
static uint16_t gapLenDn = DEFAULT_DEVICE_NAME_LEN;
...
/* Peripheral Preferred Connection characteristic */
static const uint8_t gapValPpcCh[] = {ATT_PROP_READ, UINT16_TO_BYTES(GAP_PPCP_HDL), UINT16_TO_BYTES(ATT_UUID_PREF_CONN_PARAM)};
static const uint16_t gapLenPpcCh = sizeof(gapValPpcCh);

/* Peripheral Preferred Connection */
static uint8_t gapValPpc[] = {UINT16_TO_BYTES(DEFAULT_CONN_INTERVAL_MIN), UINT16_TO_BYTES(DEFAULT_CONN_INTERVAL_MAX), UINT16_TO_BYTES(DEFAULT_CONN_LATENCY), UINT16_TO_BYTES(DEFAULT_SUPERVISION_TIMEOUT)};
static const uint16_t gapLenPpc = sizeof(gapValPpc);
...
static const attsAttr_t gapList[] =
{
  ...
  },
  {
    ...
    (CORE_SEC_PERMIT_READ | CORE_SEC_PERMIT_WRITE)
  },
  {
    attChUuid,
    (uint8_t *) gapValApCh,
...
#endif //GP_BLE_ATT_SERVER_DATABASE_HASH
...
void SvcCoreGapAddRpaoCh(void)
{
  ...
  #else
  GP_LOG_SYSTEM_PRINTF("ERROR: SvcCoreGapAddRpaoCh not supported. GP_DIVERSITY_BLE_LINK_LAYER_PRIVACY_SUPPORTED is not defined", 0);
  #endif
}
...
{
  #ifdef GP_DIVERSITY_BLE_LINK_LAYER_PRIVACY_SUPPORTED
  svcGapGroup.endHandle = GAP_RPAO_HDL - 2; //use hdl before RPAO related handles
  #else
  GP_LOG_SYSTEM_PRINTF("ERROR: SvcCoreGapRemoveRpaoCh not supported. GP_DIVERSITY_BLE_LINK_LAYER_PRIVACY_SUPPORTED is not defined", 0);
  #endif
}
