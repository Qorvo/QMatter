...
// includes needed for calculating an RPA InitA
#include "dm_main.h"
#include "dm_adv.h"
...
static void appSecReqTimerStart(dmConnId_t connId)
{
  appSlaveCb.secReqTimer.handlerId = appHandlerId;
  appSlaveCb.secReqTimer.msg.event = APP_SEC_REQUEST_TIMEOUT_IND;
  appSlaveCb.secReqTimer.msg.param = connId;
  WsfTimerStartMs(&appSlaveCb.secReqTimer, pAppSlaveCfg->securityRequestTimeout);
}

static void appSecReqTimerStop(void)
{
    WsfTimerStop(&appSlaveCb.secReqTimer);

}

static void appSlaveSecurityRequestTimeout(wsfMsgHdr_t *pMsg, appConnCb_t *pCb)
{
    DmSecSlaveReq((dmConnId_t) pCb->connId, pAppSecCfg->auth);
}
...
static void appSecRspLtk(appConnCb_t *pCb)
{
  ...
  if (pKey != NULL)
  {
    ...
    GP_LOG_SYSTEM_PRINTF("Local LTK for connId %d: 0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",0,pCb->connId,pKey->ltk.key[0],pKey->ltk.key[1],pKey->ltk.key[2],pKey->ltk.key[3],pKey->ltk.key[4],pKey->ltk.key[5],pKey->ltk.key[6],pKey->ltk.key[7],pKey->ltk.key[8],pKey->ltk.key[9],pKey->ltk.key[10],pKey->ltk.key[11],pKey->ltk.key[12],pKey->ltk.key[13],pKey->ltk.key[14],pKey->ltk.key[15]);
  }
  else
  {
    pCb->bondByLtk = FALSE;
...
static void appSlaveConnClose(dmEvt_t *pMsg, appConnCb_t *pCb)
{
  ...
  /*
  * When advertising is active changing update privacy mode is disallowed
  * AppSlaveIsAdvertising is ignoring active direct advertisements so we also check appSlaveCb.advDirected
  * if connClose.reas == 0 it means directedAdv was finished, so there is no reason change privacy mode
  * changing privacy mode should be only done for real disconnection with peer
  */
  Bool directedAdvEnded = (pMsg->connClose.reason == 0); /* reason == means that direct advertise failed due to timeout or was canceled */
  if(!directedAdvEnded && !AppSlaveIsAdvertising() && !appSlaveCb.advDirected)
  {
    AppUpdatePrivacyMode(pCb->dbHdl);
  }
  /*
  * If advertise is active updating peers privacy mode will be done in special way in BleModule_Peripheral
  * DM_CONN_CLOSE_IND implementation in BleModule_Peripheral will enqueue requied setting privacy mode events
  * Proper setting privacy mode will be realized later - when system will stop advertising
  */
...
#ifdef GP_DIVERSITY_CORDIO_QORVO_APP
    while(appSlaveCb.dbHdl != APP_DB_HDL_NONE)
    {
        ...
        /* If End of Database, trigger bind with new RPA address */
        if(appSlaveCb.dbHdl == APP_DB_HDL_NONE)
        {
          /* Need to handle Reconnect from unknown RPA */
          cordioAppFramework_cbConnOpen(pMsg, NULL, true);
...
#ifdef GP_DIVERSITY_CORDIO_QORVO_APP
  /* Reconnected to known RPA-reconnected, trigger cbConnOpen */
  cordioAppFramework_cbConnOpen(pMsg, pCb->dbHdl, false);
#endif
  /* done with this address resolution */
  appSlaveCb.inProgress = FALSE;
}
...
void appSlaveSecConnOpen(dmEvt_t *pMsg, appConnCb_t *pCb)
{
  ...
  if(pCb->dbHdl != NULL)
    {
      /* Trigger callback for reconnecting */
      cordioAppFramework_cbConnOpen(pMsg, pCb->dbHdl, false);
    }
#endif // GP_DIVERSITY_CORDIO_QORVO_APP
  ...
  if (pCb->dbHdl == NULL)
  {
    /* if peer device uses an RPA */
    if (DM_RAND_ADDR_RPA(pMsg->connOpen.peerAddr, pMsg->connOpen.addrType))
    {
      /* resolve master's RPA to see if we already have a bond with this device */
      appSlaveResolveAddr(pMsg);
#ifdef GP_DIVERSITY_CORDIO_QORVO_APP
      /* Check In progress, if in progress is cleared, we have a new RPA based connection. */
      if(appSlaveCb.inProgress == false)
      {
        cordioAppFramework_cbConnOpen(pMsg, NULL, true);
      }
#endif // GP_DIVERSITY_CORDIO_QORVO_APP
    ...
    {
      /* If dbHdl == NULL, and not a resolvable address, then trigger callBack in BleModule
       What if we recieve a non-resolvable address? */
      cordioAppFramework_cbConnOpen(pMsg, NULL, true);
    }
#endif // GP_DIVERSITY_CORDIO_QORVO_APP
  }
  ...
  if (pAppSecCfg->initiateSec)
  {
    /* DmSecSlaveReq((dmConnId_t) pMsg->hdr.param, pAppSecCfg->auth); */
    appSecReqTimerStart((dmConnId_t) pMsg->hdr.param);
  }
}
...
#ifdef GP_DIVERSITY_CORDIO_QORVO_APP
      /* Due to strategy on Connection maintenace, it is possible there is no dbRecord available,
         In that case, drop pair request and connection */
      if(pCb->dbHdl == APP_DB_HDL_NONE)
      {
...
void appSlaveProcMsg(wsfMsgHdr_t *pMsg)
{
  ...
  switch(pMsg->event)
  {
    case APP_CONN_UPDATE_TIMEOUT_IND:
      ...
      break;
    case APP_SEC_REQUEST_TIMEOUT_IND:
      appSlaveSecurityRequestTimeout(pMsg,pCb);
      break;
    default:
      break;
  }
       ...
       (advType == DM_ADV_CONN_DIRECT_LO_DUTY)))
  {
    ...
    if ((interval > 0) || (advType == DM_ADV_CONN_DIRECT))
    {
      ...
      if (DmLlPrivEnabled() || DM_RAND_ADDR_RPA(pAddr, addrType))
      {
        ...
        /* If peer was added to resolving list but has not address resolution support directed advertise will be blocked */
        /* if peer was not added to resolving list then RPAs (for AdvA and TargetA) wont be used so there is no need to block */
        if ((dbHdl == APP_DB_HDL_NONE) || (!AppDbGetPeerAddrRes(dbHdl) && AppDbGetPeerAddedToRl(dbHdl)))
        {
