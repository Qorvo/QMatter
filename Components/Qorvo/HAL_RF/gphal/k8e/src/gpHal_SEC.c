 ...
 *
 * $Header: //depot/release/Embedded/Components/Qorvo/HAL_RF/v2.10.2.1/comps/gphal/k8e/src/gpHal_SEC.c#1 $
 * $Change: 189026 $
...
{
    gpHal_Result_t halResult = gpHal_ResultInvalidRequest;
    GP_LOG_SYSTEM_PRINTF("gpHal_HMAC implemented but not tested!", 0);
    GP_ASSERT_SYSTEM(0);

#ifdef GP_HAL_DIVERSITY_SEC_CRYPTOSOC
    sx_enable_clock();

    UInt32 result = sx_hmac_blk((sx_hash_fct_t)hashFct,
                                  block_t_convert(pKey, keyLength),
                                  block_t_convert(pMsg, msgLength),
                                  block_t_convert(pResult, resultLength));

    halResult = (result == CRYPTOLIB_SUCCESS) ? gpHal_ResultSuccess : gpHal_ResultInvalidRequest;

    sx_disable_clock();
#else
    // should be implemented
    halResult = gpHal_ResultUnsupported;
#endif //GP_HAL_DIVERSITY_SEC_CRYPTOSOC
    return halResult;
}
