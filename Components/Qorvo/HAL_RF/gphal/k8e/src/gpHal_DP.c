 ...
 *
 * $Header: //depot/release/Embedded/Components/Qorvo/HAL_RF/v2.10.2.1/comps/gphal/k8e/src/gpHal_DP.c#1 $
 * $Change: 189026 $
    ...
    if(dumpShort)
    {
        ...
        for(i = 0; i < GPHAL_DP_SHORT_LIST_MAX; i++)
        {
            ...
            if(valid)
            {
                ...
                GP_LOG_SYSTEM_PRINTF("%i:%x %x",0,i, panId, shortAddr);
                gpLog_Flush();
            }
    ...
    else
    {
        ...
        for(i = 0; i < GPHAL_DP_LONG_LIST_MAX; i++)
        {
            ...
            if(valid)
            {
                ...
                GP_LOG_SYSTEM_PRINTF("%i:%x %lx %lx",0,i, panId, (long unsigned int)longAddr.MSB, (long unsigned int)longAddr.LSB);
                gpLog_Flush();
            }
