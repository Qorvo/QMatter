...
gpHal_Result_t gpHal_BlePopulateInitEventInfo(gpHal_InitEventInfo_t* pInfo)
{
    ...
    {
        GP_WB_WRITE_INIT_COMMON_EXT_INFO_DEPR_PRI_PHY_MASK(GP_HAL_BLE_EXT_INIT_INFO_START, pInfo->initPhyMask.mask & 0xFD); // mask out 2Mbit
    }
...
Bool gpHal_BleCompareEventParams(gpHal_BleServiceEventId_t serviceEvent, gpHal_phyMask_t mask_new)
{
    ...
    switch (serviceEvent)
    {
        ...
        case gpHal_BleServiceEventIdInitiating:
            ...
            {
                mask_old = GP_WB_READ_INIT_COMMON_EXT_INFO_DEPR_PRI_PHY_MASK(GP_HAL_BLE_EXT_INIT_INFO_START);
            }
            break;
        default:
...
void gpHal_BlePopulateBgscEventInfo(gpHal_Address_t bgscInfoAddress, gpHal_BleServiceEventId_t serviceEvent, UInt8 service_flags, gpHal_phyMask_t phyMask)
{
    ...
    switch (serviceEvent)
    {
        ...
        case gpHal_BleServiceEventIdInitiating:
            ...
            {
                GP_WB_WRITE_INIT_COMMON_EXT_INFO_DEPR_PRI_PHY_MASK(GP_HAL_BLE_EXT_INIT_INFO_START, phyMask.mask & 0xFD); // mask out 2Mbit
            }
    ...
    if(gpHal_GetRtSystemVersion(gpHal_RtSubSystem_BleMgr) < GP_HAL_BLE_FIRST_RT_VERSION_SUPPORTING_SEC_PHY_MASK)
    {
        GP_WB_WRITE_BGSC_EV_INFO_INIT_COMMON_EXT_INFO_BASE_PTR_DEPR(bgscInfoAddress,TO_GPM_ADDR(GP_HAL_BLE_EXT_INIT_INFO_START));
        GP_WB_WRITE_BGSC_EV_INFO_SCAN_COMMON_EXT_INFO_BASE_PTR_DEPR(bgscInfoAddress,TO_GPM_ADDR(GP_HAL_BLE_EXT_SCAN_INFO_START));
    }
...
#ifndef GP_COMP_CHIPEMU
        // Initialize the desense fix. This will populate gpHal_BleDsFixRtScratchArray
    ...
    if (new_flags)
    {
        ...
        switch (serviceEvent)
        {
            ...
            case gpHal_BleServiceEventIdInitiating:
                ...
                {
                    phyMask.mask = GP_WB_READ_INIT_COMMON_EXT_INFO_DEPR_PRI_PHY_MASK(GP_HAL_BLE_EXT_INIT_INFO_START); // we can only copy the existing mask, we cannot restore the one originally set with the service
                }
                break;
            default:
                GP_ASSERT_SYSTEM(false); /* invalid param */
...
gpHal_Result_t gpHal_BleRestartInitScanning(UInt32 firstInitScanTs, gpPd_Loh_t ConnectReqPdu)
{
    ...
    {
        phyMask.mask = GP_WB_READ_INIT_COMMON_EXT_INFO_DEPR_PRI_PHY_MASK(GP_HAL_BLE_EXT_INIT_INFO_START);
    }
