...
void gpHalReset_InitMacFilter(void)
{
    ...
    GP_WB_WRITE_MACFILT_FT_ACK_TO_QUEUE(true);
...
#if defined(GP_HAL_DIVERSITY_RAW_ENHANCED_ACK_RX)
    GP_WB_WRITE_MACFILT_ENABLE_2015_SUPPORT(true);
    GP_WB_WRITE_MACFILT_ACCEPT_FV_2015(true);
    GP_WB_WRITE_RIB_ACK_TIMEOUT(GP_HAL_ENHACK_TIMEOUT);
#else
...
void gpHal_Init(Bool timedMAC)
{
    ...
    GP_WB_WRITE_TRX_EXT_MODE_CTRL_FOR_RX_ON_ATT_CTRL_LOW(BM(GP_BSP_RF_DEBUG_RX_MODE_CTRL_BIT));
    GP_WB_WRITE_TRX_EXT_MODE_CTRL_FOR_RX_ON_ATT_CTRL_HIGH(BM(GP_BSP_RF_DEBUG_RX_MODE_CTRL_BIT));
