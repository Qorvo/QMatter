...
bool_t dmAdvPauseReq(void)
{
    if (DM_ADV_NONE != dmAdvCb.advType[DM_ADV_HANDLE_DEFAULT])
    {
...
void dmAdvResume(void)
{
    HciLeSetAdvEnableCmd(TRUE);
}

bool_t dmAdvStarted(void)
{
    return (DM_ADV_NONE != dmAdvCb.advType[DM_ADV_HANDLE_DEFAULT]);
}
