...
uint8_t attsPermissions(dmConnId_t connId, uint8_t permit, uint16_t handle, uint8_t permissions)
{
  ...
  {
    /* if no LTK available */
    if (attsCb.ltkAvail[connId - 1] == FALSE)
    {
      /* When the link is not encrypted, the error code 'Insufficient Authentication' does not
        indicate that MITM protection is required. */
      return ATT_ERR_AUTH;
    }
