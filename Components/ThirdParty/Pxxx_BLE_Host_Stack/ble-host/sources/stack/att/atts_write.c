...
static uint8_t attsExecPrepWrite(attsCcb_t *pCcb, attsPrepWrite_t *pPrep)
{
           ...
           (pGroup->writeCback != NULL))
  {
    ...
    attsCb.writeCbackSlot = pCcb->slot;
 ...
 *
 *  \note   This function can only be called from the write callback.
 */
/*************************************************************************************************/
uint8_t AttsWriteCbackSlot(void)
{
  return attsCb.writeCbackSlot;
}
...
void AttsContinuePrepWriteReq(dmConnId_t connId, uint16_t handle, uint8_t status, uint8_t offset, uint8_t *pValue, uint16_t len)
{
  ...
  if (status)
  {
    attErrRsp(pCcb, ATT_BEARER_SLOT_ID, ATT_PDU_PREP_WRITE_REQ, handle, status);
  }
  else
  {
    /* allocate response buffer */
    if ((pBuf = attMsgAlloc(L2C_PAYLOAD_START + ATT_PREP_WRITE_RSP_LEN + len)) != NULL)
    {
      /* build and send PDU */
      p = pBuf + L2C_PAYLOAD_START;
      UINT8_TO_BSTREAM(p, ATT_PDU_PREP_WRITE_RSP);
      ...
      UINT16_TO_BSTREAM(p, offset);
      memcpy(p, pValue, len);
      attL2cDataReq(pCcb, ATT_BEARER_SLOT_ID, (ATT_PREP_WRITE_RSP_LEN + len), pBuf);
    }
  }
}
...
void AttsContinueExecWriteReq(dmConnId_t connId, uint8_t status)
{
  ...
  if (status)
  {
    attErrRsp(pCcb, ATT_BEARER_SLOT_ID, ATT_PDU_EXEC_WRITE_REQ, 0, status);
  }
  else
  {
    if ((pBuf = attMsgAlloc(L2C_PAYLOAD_START + ATT_EXEC_WRITE_RSP_LEN)) != NULL)
    {
      ...
      p = pBuf + L2C_PAYLOAD_START;
      UINT8_TO_BSTREAM(p, ATT_PDU_EXEC_WRITE_RSP);
      attL2cDataReq(pCcb, ATT_BEARER_SLOT_ID, ATT_EXEC_WRITE_RSP_LEN, pBuf);
    }
  }
}
