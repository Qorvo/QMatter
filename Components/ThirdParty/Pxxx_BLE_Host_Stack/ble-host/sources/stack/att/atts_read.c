...
void AttsContinueReadReq(dmConnId_t connId, uint16_t handle, uint8_t status, uint8_t *pValue, uint16_t len)
{
  ...
  /* determine length of data to read */
  readLen = (len < (pCcb->sccb[ATT_BEARER_SLOT_ID].mtu - ATT_READ_RSP_LEN)) ?
             len : (pCcb->sccb[ATT_BEARER_SLOT_ID].mtu - ATT_READ_RSP_LEN);
...
void AttsContinueReadBlobReq(dmConnId_t connId, uint16_t handle, uint8_t status, uint16_t offset, uint8_t *pValue, uint16_t len)
{
  ...
  /* determine length of data to read */
  readLen = (len - offset  < (pCcb->sccb[ATT_BEARER_SLOT_ID].mtu - ATT_READ_BLOB_RSP_LEN)) ?
             len - offset : (pCcb->sccb[ATT_BEARER_SLOT_ID].mtu - ATT_READ_BLOB_RSP_LEN);
