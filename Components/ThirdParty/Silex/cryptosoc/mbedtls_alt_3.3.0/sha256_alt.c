                          ...
                          size_t ilen)
{
    ...
    while(ilen >= 64)
    {
        ...
        ret = sx_hash_update_blk(hash_algo, block_t_convert(ctx->state, 32), block_t_convert(input, steplen));
...
int mbedtls_sha256_finish(mbedtls_sha256_context* ctx, unsigned char* output)
{
    ...
    ret = mbedtls_sha256_update(ctx, sha256_padding, padn);
    ret |= mbedtls_sha256_update(ctx, msglen, 8);
