                                  ...
                                  const unsigned char* buf, size_t ilen)
{
    ...
    MBEDTLS_MPI_CHK(mbedtls_mpi_read_binary(&pt->MBEDTLS_PRIVATE(Y), buf + 1 + plen, plen));
    MBEDTLS_MPI_CHK(mbedtls_mpi_lset(&pt->MBEDTLS_PRIVATE(Z), 1));
                       ...
                       const mbedtls_mpi* m, const mbedtls_ecp_point* P)
{
    ...
    MBEDTLS_MPI_CHK(mbedtls_mpi_write_binary(&P->MBEDTLS_PRIVATE(X), point, opsize));
    ...
    MBEDTLS_MPI_CHK(mbedtls_mpi_read_binary(&R->MBEDTLS_PRIVATE(X), result, opsize));
...
int mbedtls_ecp_group_load(mbedtls_ecp_group* grp, mbedtls_ecp_group_id id)
{
    ...
    mbedtls_mpi_read_binary(&grp->G.MBEDTLS_PRIVATE(X), curve->params.addr + 2 * opsize, opsize);
    mbedtls_mpi_read_binary(&grp->G.MBEDTLS_PRIVATE(Y), curve->params.addr + 3 * opsize, opsize);
    mbedtls_mpi_lset(&grp->G.MBEDTLS_PRIVATE(Z), 1);
                       ...
                       int (*f_rng)(void*, unsigned char*, size_t), void* p_rng)
{
    ...
    do
    {
        ...
        if(!sx_bignum_is_zero(rnd, opsize) && sx_bignum_compare(rnd, curve->params.addr + opsize, opsize) < 0)
        {
            sx_enable_clock();
