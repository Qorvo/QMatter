...
int mbedtls_internal_sha1_process(mbedtls_sha1_context* ctx, const unsigned char data[64])
{
    ...
    sx_enable_clock();
    ret = sx_hash_update_blk(e_SHA1, block_t_convert(ctx->state, sizeof(ctx->state)), block_t_convert(data, 64));
    sx_disable_clock();
...
int mbedtls_sha1_update_ret(mbedtls_sha1_context* ctx, const unsigned char* input, size_t ilen)
{
    ...
    while(ilen >= 64)
    {
        ...
        steplen &= ~0x3F;
        sx_enable_clock();
        ret = sx_hash_update_blk(e_SHA1, block_t_convert(ctx->state, sizeof(ctx->state)), block_t_convert(input, steplen));
        sx_disable_clock();
        if(ret)
