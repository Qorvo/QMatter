...
#if defined(GP_DIVERSITY_BLE_PERIPHERAL) || defined(GP_DIVERSITY_BLE_BROADCASTER) || defined(GP_DIVERSITY_BLE_LEGACY_ADVERTISING)
#include "gpBleAdvertiser.h"
#endif // GP_DIVERSITY_BLE_PERIPHERAL || GP_DIVERSITY_BLE_BROADCASTER || GP_DIVERSITY_BLE_LEGACY_ADVERTISING
...
uint8_t LlGetResolvingListSize(void)
{
    ...
    (void)gpBle_LeReadResolvingListSize(&cmd, &evt);

    return evt.payload.commandCompleteParams.returnParams.resolvingListSize;
#else
    return 0;
#endif
}
...
uint8_t HciGetWhiteListSize(void)
{
    ...
    gpBle_LeReadFilterAcceptListSize(&cmd, &evt);

    return evt.payload.commandCompleteParams.returnParams.filterAcceptListSize;
}
