...
void HciLeReadPhyCmd(uint16_t handle)
{
    ...
    gpBle_ExecuteCommand(gpBle_EventServer_Host, gpHci_OpCodeLeReadPhy, gpBle_LeReadPhy, &cmd);
#else
...
#ifdef GP_DIVERSITY_BLE_PHY_UPDATE_SUPPORTED
  gpBle_ExecuteCommand(gpBle_EventServer_Host, gpHci_OpCodeLeSetDefaultPhy, gpBle_LeSetDefaultPhy, &cmd);
#else
...
#ifdef GP_DIVERSITY_BLE_PHY_UPDATE_SUPPORTED
  gpBle_ExecuteCommand(gpBle_EventServer_Host, gpHci_OpCodeLeSetPhy, gpBle_LeSetPhy, &cmd);
#else
