...
defined(GP_DIVERSITY_BLE_SCANNER) || defined (GP_DIVERSITY_BLE_SCAN_COEX_SUPPORTED)
...
void HciLeAddDevWhiteListCmd(uint8_t addrType, uint8_t *pAddr)
{
    ...
    cmd.LeAddDeviceToWhiteList.addressType = addrType;
    MEMCPY(&cmd.LeAddDeviceToWhiteList.address.addr, pAddr, sizeof(cmd.LeAddDeviceToWhiteList.address.addr));

    gpBle_ExecuteCommand(gpBle_EventServer_Host, gpHci_OpCodeLeAddDeviceToWhiteList, gpBle_LeAddDeviceToWhiteList, &cmd);
}
...
void HciLeClearWhiteListCmd(void)
{
    ...
    gpBle_ExecuteCommand(gpBle_EventServer_Host, gpHci_OpCodeLeClearWhiteList, gpBle_LeClearWhiteList, &cmd);
}
...
void HciLeRandCmd(void)
{
    ...
    evt.hdr.status = gpBle_LeRand(&cmd, &evtCmd);
...
void HciLeReadWhiteListSizeCmd(void)
{
  /* unused */
    ...
    gpBle_ExecuteCommand(gpBle_EventServer_Host, gpHci_OpCodeLeReadWhiteListSize, gpBle_LeReadWhiteListSize, &cmd);
}
...
void HciLeRemoveDevWhiteListCmd(uint8_t addrType, uint8_t *pAddr)
{
    ...
    cmd.LeRemoveDeviceFromWhiteList.addressType = addrType;
    MEMCPY(&cmd.LeRemoveDeviceFromWhiteList.address, pAddr, sizeof(cmd.LeCreateConnection.peerAddress));

    gpBle_ExecuteCommand(gpBle_EventServer_Host, gpHci_OpCodeLeRemoveDeviceFromWhiteList, gpBle_LeRemoveDeviceFromWhiteList, &cmd);
}
...
void HciLeSetHostChanClassCmd(uint8_t *pChanMap)
{
    ...
    MEMCPY(cmd.LeSetHostChannelClassification.channels.channels, pChanMap, GP_HCI_CHANNEL_MAP_LENGTH);

    gpBle_ExecuteCommand(gpBle_EventServer_Host, gpHci_OpCodeLeSetHostChannelClassification, gpBle_LeSetHostChannelClassification, &cmd);
#else
